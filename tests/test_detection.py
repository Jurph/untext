...
def test_doctr_detector_on_null_image(null_image):
    ...
    detections = detect_with_doctr(null_image, confidence_threshold=0.3)
...
def test_easyocr_detector_on_null_image(null_image):
    ...
    detections = detect_with_easyocr(null_image, confidence_threshold=0.3)
...
def test_east_detector_on_null_image(null_image):
    ...
    detections = detect_with_east(null_image, confidence_threshold=0.3)
...
def test_all_detectors_on_test_image(test_images):
    ...
    doctr_detections = detect_with_doctr(image, confidence_threshold=0.3)
    easyocr_detections = detect_with_easyocr(image, confidence_threshold=0.3) 
    east_detections = detect_with_east(image, confidence_threshold=0.3)
    ...
    for detections, detector_name in [
        ...
        for i, detection in enumerate(detections):
            ...
            # detect_with_* functions return confidence_pct (0-100), not fraction
            assert 0 <= conf <= 100, f"{detector_name} detection {i} confidence should be 0-100"
