...
Divide the image into a **2×3 or 3×2 grid** (6 zones): the long edge is subdivided into thirds, the short edge into halves. For a landscape image this yields 3 columns × 2 rows; for portrait, 2 columns × 3 rows. This resolution captures corner-marking strategies without risking a watermark straddling a zone boundary. For each draw, each extracted blob independently contributes to the zone containing its centroid. A draw that produces two blobs populates two zones. Convergence is tracked globally across the full set of occupied zones (not per-zone). Zero-blob draws count toward the consecutive-stable-draw streak — they do not change the set, so they count as stable. Continue drawing until either: (a) the set of occupied zones has not changed for **10 consecutive draws**, or (b) **50 draws** total have been performed. If 50 draws complete without convergence, accept the current occupied set and continue (log a warning). Each occupied zone at convergence represents one candidate marking strategy (e.g., upper-right, lower-left).
